...
public class SendMoneyAdapter extends RecyclerView.Adapter {
    ...
    private List<SendMoneyEntity.DataBean.ListBean> list;
    private static int PL = 0;
    private static int BL = 1;
    private static int RB = 2;

    public SendMoneyAdapter(Context context, List<SendMoneyEntity.DataBean.ListBean> list) {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        if (viewType == PL) {
            ...
            LayoutInflater.from(context).inflate(R.layout.dynamic_pl_item, null);
            return new PLViewHolder(view);

        } else if (viewType == BL) {

            LayoutInflater.from(context).inflate(R.layout.dynamic_bl_item, null);
            return new BLViewHolder(view);
        } else {

            LayoutInflater.from(context).inflate(R.layout.dynamic_rb_item, null);
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        ...
        if (list.get(position).getType() == 1 || list.get(position).getType() == 4) {
            ...
            Glide.with(context).load(list.get(position).getImage()).into(bl.dynamic_bl_img);
            ...
            Glide.with(context).load(list.get(position).getImage()).into(rb.dynamic_rb_img);
